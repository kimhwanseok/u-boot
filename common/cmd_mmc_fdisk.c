...
#define		USER_DATA_PART_SIZE		_300MB
#define		CACHE_PART_SIZE			_256MB
...
int calc_unit(u64 length, SDInfo sdInfo)
{
	if (sdInfo.addr_mode == CHS_MODE)
		return ((div_64bit(div_64bit(length, BLOCK_SIZE), sdInfo.unit) + 1) * sdInfo.unit);
	else
		return ( div_64bit(length, BLOCK_SIZE) );
}
...
///////////////////////////////////////////////////////////	
	memset(mbr, 0x00, sizeof(*mbr)*512);
